...
#define APP_FLASH_SECTORS (                                             \
	FLASH_A_SECTOR0  | FLASH_A_SECTOR1  | FLASH_A_SECTOR2  | FLASH_A_SECTOR3  | \
    FLASH_A_SECTOR4  | FLASH_A_SECTOR5  | FLASH_A_SECTOR6  | FLASH_A_SECTOR7  | \
    FLASH_A_SECTOR8  | FLASH_A_SECTOR9  | FLASH_A_SECTOR10 | FLASH_A_SECTOR11 |	\
    FLASH_A_SECTOR12 | FLASH_A_SECTOR13 | FLASH_A_SECTOR14 | FLASH_A_SECTOR15 )
#endif
...
#if defined(__MCU_HAS_FLCTL_A__)
	MAP_FlashCtl_A_unprotectMemory(FLASH_APP_START, FLASH_APP_START+FLASH_APP_SIZE);
	ret = MAP_FlashCtl_A_performMassErase();
#endif
#if defined(__MCU_HAS_FLCTL__)
...
uint32_t on_foe_close_cb( void ) {
	...
	MAP_FlashCtl_A_getMemoryInfo((uint32_t)&CRC_App, &bankNum, &sectorNum);
    DPRINT("\n%s crc 0x%04X addr 0x%04X bank %d sector %d\n",
    		__FUNCTION__, gCalc_crc, &CRC_App, bankNum, sectorNum);
	// unprotect bank and sector ...
    MAP_FlashCtl_A_unprotectMemory((uint32_t)&CRC_App, (uint32_t)&CRC_App);
	// erase & program
	MAP_FlashCtl_A_eraseSector((uint32_t)&CRC_App);
	ret = MAP_FlashCtl_A_programMemory((void*)&gCalc_crc, (void*)&CRC_App, sizeof(gCalc_crc));
	// protect
	MAP_FlashCtl_A_protectMemory((uint32_t)&CRC_App, (uint32_t)&CRC_App);
	MAP_FlashCtl_A_protectMemory(FLASH_APP_START, FLASH_APP_START+FLASH_APP_SIZE);
#endif
#if defined(__MCU_HAS_FLCTL__)
	// get flash bank and sector of crc address ....
